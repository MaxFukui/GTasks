...
class TaskService:
    ...
    def sync_from_google(self):
        ...
        for task_list in task_lists:
            ...
            tasks = (
                self.service.tasks()
                .list(tasklist=list_id, showHidden=True)
                .execute()
                .get("items", [])
            )
            if tasks:
    ...
    def _cascade_complete(self, list_id, parent_id):
        ...
        child_tasks_ids = [
            task["id"] for task in tasks if task.get("parent") == parent_id
        ]
    ...
    def move_task(self, source_list_id, target_list_id, task_id):
        """Moves a task from source list to target list in the local cache."""
        if not source_list_id or not target_list_id or not task_id:
            return None

        if source_list_id == target_list_id:
            return None

        task = self.get_task(source_list_id, task_id)
        if not task:
            return None

        self.add_task_body(target_list_id, task)
            ...
            if task_list.get("deleted"):
                ...
                if google_list and task_list.get("title") != google_list.get("title"):
                    self.service.tasklists().patch(
                        tasklist=task_list["id"], body={"title": task_list.get("title")}
                    ).execute()

        # Remove deleted lists from local cache
        self.data["task_lists"] = [
            lst for lst in self.data["task_lists"] if not lst.get("deleted")
        ]
            ...
            while unprocessed_new_tasks:
                ...
                for task in unprocessed_new_tasks:
                    ...
                    if not parent_id or new_parent_id:
                        ...
                        if "status" in task:
                            new_task_body["status"] = task["status"]
                        ...
                        task["id"] = new_task[
                            "id"
                        ]  # Update the local task with the new ID
                ...
                if processed_in_this_pass == 0 and remaining_tasks:
                    ...
                    for task in remaining_tasks:
                        ...
                        new_task = (
                            self.service.tasks()
                            .insert(tasklist=list_id, body=new_task_body)
                            .execute()
                        )
                        task["id"] = new_task["id"]
                    break
            ...
            def update_children(parent_id):
                for task in local_tasks_list:
                    if task.get("parent") == parent_id:
                        if (
                            not task["id"].startswith("temp_")
                            and task["id"] in google_tasks_map
                        ):
                            google_task = google_tasks_map[task["id"]]

                            update_body = {}
                            if task.get("title") != google_task.get("title"):
                                update_body["title"] = task.get("title")
                            if task.get("notes") != google_task.get("notes"):
                                update_body["notes"] = task.get("notes")
                            if task.get("due") != google_task.get("due"):
                                update_body["due"] = task.get("due")
                            if task.get("status") != google_task.get("status"):
                                update_body["status"] = task.get("status")

                            if update_body:
                                self.service.tasks().patch(
                                    tasklist=list_id, task=task["id"], body=update_body
                                ).execute()
