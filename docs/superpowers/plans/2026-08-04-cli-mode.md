...
- Produces:
  ...
  - `render.render(rows: list[dict], mode: str, group_by_list: bool, sync_info: dict | None = None, today: datetime.date | None = None) -> str` — `today` added during Task 5 review so the overdue-red branch is pinnable in tests; defaults to `datetime.date.today()`. Task 6 does not pass it.
...
def main():
    ...
    # Any argument at all means the CLI. Matching against a verb list here
    # would send a typo'd verb to the TUI, which ignores sys.argv — the user
    # would get a full-screen UI instead of "invalid choice: 'favs'".
    # (Amended during Task 6 review; the original predicate also matched
    # argv[0] against cli.VERBS, which has since been removed.)
    is_cli = bool(argv)
    if is_cli:
        from . import cli
